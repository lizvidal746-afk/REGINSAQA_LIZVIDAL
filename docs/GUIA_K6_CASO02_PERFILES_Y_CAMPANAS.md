...
## Pack 1: smoke / burst medio / burst alto (2 / 10 / 50)

### Smoke 2 (recomendado)

```powershell
k6 run tests/performance/k6-grafana/k6_caso_02_registrar_sancion.js `
  --tag test_type=smoke --tag profile=baseline --tag case=caso02 --tag mode=no-burst --tag size=2 `
  --env BASE_API=$env:BASE_API --env TOKEN1=$env:TOKEN1 --env K6_BURST_MODE=0 `
  --env K6_FIXED_VUS=1 --env K6_FIXED_ITERATIONS=2 --env K6_TOTAL_REGISTROS=2 `
  --env K6_SLEEP_SECONDS=0.8 --summary-export reportes/k6-caso02-smoke-2-summary-cloud.json

npm run k6:caso02:reporte:auto -- --summary=reportes/k6-caso02-smoke-2-summary-cloud.json --campaign=campana-caso02-1 --test-type=smoke --profile=baseline --mode=no-burst --size=2
```

> Nota: si ejecutas `K6_TOTAL_REGISTROS=3`, el Caso 02 aplica secuencia de detalle `2,1,2...` por registro en modo `sequence`.
