...
if (-not $env:REGINSA_VERIFY_WAIT_MS) {
  $env:REGINSA_VERIFY_WAIT_MS = '1000'
}
...
if (-not $env:REGINSA_REGISTRO_TIMEOUT_MS) {
  $env:REGINSA_REGISTRO_TIMEOUT_MS = '120000'
}
...
if ($requestedWorkers -ge 8) {
  Set-MinIntEnv -Name 'REGINSA_REGISTRO_TIMEOUT_MS' -Min 180000
