...
function Remove-WorkersArgs {
  ...
  for ($i = 0; $i -lt $InputArgs.Count; $i++) {
    ...
    if ($arg -eq '--workers') {
      if (($i + 1) -lt $InputArgs.Count -and $InputArgs[$i + 1] -match '^\d+$') {
        $i++
      }
      continue
    }
...
if (-not $isCi -and -not $allowOvercommit -and $requestedWorkers -gt $maxWorkersLocal) {
  Write-Host "⚠️ Sobrecarga detectada: workers solicitados=$requestedWorkers, max local recomendado=$maxWorkersLocal (CPU lógicos=$cpuCount)." -ForegroundColor Yellow
  ...
  $effectiveWorkers = $maxWorkersLocal
}
