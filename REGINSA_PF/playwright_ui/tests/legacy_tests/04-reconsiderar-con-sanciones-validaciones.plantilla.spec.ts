...
async function abrirRegistroElegible(page: Page): Promise<Date | null> {
  ...
  if (idxFMod < 0 || idxNRec < 0 || idxFRec < 0) {
    throw new Error('No se pudieron identificar columnas de reconsideracion para validaciones de Caso 04.');
  }
...
async function completarCamposCabecera(scope: Locator, fechaResolucion: Date | null, omitir: OmitirCampo): Promise<void> {
  ...
  if (omitir !== 'numero') {
    ...
    await inputNumero.fill(`RECONS-VAL-04-${Date.now()}`);
  }
...
test.describe('@validaciones 04-RECONSIDERAR CON SANCIONES - Cabecera obligatoria', () => {
  ...
  for (const item of casos) {
    test(`bloquea guardado cuando falta: ${item.titulo}`, async ({ page }, testInfo) => {
      ...
      test.skip(!fechaResolucion, 'No hay registro elegible para validar Caso 04.');
